...
public class MaxAffinityRankCompetitionTest extends BCellSelectionTestBase {
    static {
        System.setProperty(EpitopeCaptureModel.MODEL_TYPE_PROPERTY, "CK");
        System.setProperty(TCellCompetitionModel.MODEL_TYPE_PROPERTY, "MAX_AFFINITY_RANK");
    ...
    @Test public void testCompetition() {
        ...
        double meanSurvived = survived.stream().mapToDouble(bcell -> bcell.getMaxAffinity()).average().orElse(Double.NaN);
        double meanPerished = perished.stream().mapToDouble(bcell -> bcell.getMaxAffinity()).average().orElse(Double.NaN);

        assertEquals(300, perished.size());
        assertEquals(700, survived.size());

        assertTrue(meanSurvived > meanPerished);
        assertTrue(weakestSurvived.getMaxAffinity() >= strongestPerished.getMaxAffinity());
    }

    public static void main(String[] args) {
        org.junit.runner.JUnitCore.main("amat.junit.MaxAffinityRankCompetitionTest");
    }
}
